...
# Directories
DATA_DIR = "D:/KP Apps/MonarqAI/final flourish/data/"
PROCESSED_DIR = "D:/KP Apps/MonarqAI/final flourish/data/processed/"
RAW_DIR = "D:/KP Apps/MonarqAI/final flourish/data/raw/"
...
class DataLoader:
    ...
    def load_available_data(self):
        ...
        if os.path.exists(integrated_path):
            try:
                ...
                print(f"Could not load integrated dataset: {e}")
        ...
        for econ_path in econ_paths:
            if os.path.exists(econ_path):
                try:
                    ...
                    print(f"Could not load economic data from {econ_path}: {e}")
                    continue
    ...
    def create_unified_dataset(self):
        ...
        if 'dhaka_cases' in df.columns and 'dhaka_population_estimated' in df.columns:
            ...
            print("Missing required columns for cases_per_100k calculation")
            df['cases_per_100k'] = 0
            df['severity'] = 'Unknown'
...
def try_earth_engine():
    ...
    if not EE_AVAILABLE:
        print("Earth Engine not available - skipping satellite analysis")
        return None
        
    try:
        # Initialize with your authenticated project
        ee.Initialize(project=GCP_PROJECT_ID)
        print(f"\nEarth Engine connected to project: {GCP_PROJECT_ID}")
        ...
        if mean_radiance and 'avg_rad' in mean_radiance:
            ...
            print("Could not retrieve nightlight data")
            return None
...
def main():
    ...
    print(f"Earth Engine integration with project: {GCP_PROJECT_ID}")
